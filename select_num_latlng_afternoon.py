...
for item in df_list:
    ...
    df =df.ix[(df.conntime/1000) > 1492401600] # after 12:00
    ...
    # df.lng.min(), df.lng.max(), df.lat.min(), df.lat.max()
    # plt.scatter(df.lng, df.lat, color='r')
    # plt.show()
    ...
    filename = 'F:/ChinaVis/trace_phone/0417_2/trace-%s.txt' % phoneSearch
